...
if option=="Check for a single course":
    with st.form(key="my_form"):
        ...
        if submit_button:
            ...
            st.success(f"Output for the course '{singleCourse}'")
...
# If user uploads a file
    ...
    if uploaded_file:
        ...
        try:
            courseToPredict = pd.read_csv(uploaded_file, engine='openpyxl').iloc[:, 0].drop_duplicates().str.lower().str.strip()
        # Execute if xlsx file is uploaded
        except:
            courseToPredict = pd.read_excel(uploaded_file, engine='openpyxl').iloc[:, 0].drop_duplicates().str.lower().str.strip()

        # Out a text
        st.markdown("**Making prediction. Please wait..**")
        ...
        # Output the df
        st.write(matchedDf)
        st.success(f"Successfully predicted {matchedDf.shape[0]} records!")
